...
final class AdminSession extends Model
{
    ...
    public function getDeviceAttribute(): string
    {
        ...
        // If device detected as desktop but User-Agent has mobile markers
        if ($deviceName === 'desktop' || $deviceName === '') {
            ...
            // No mobile markers: use brand or generic desktop label
            if ($brand) {
                return "{$brand} Desktop";
            }
            // If device name is empty string and no mobile markers, check if User-Agent has any content
            if (!preg_match('/\S/', $ua)) {
                return 'Desconocido';
            }
            return 'Desktop';
    ...
    public function getPlatformAttribute(): string
    {
        ...
        // If DeviceDetector didn't detect the OS, try to extract from UA
        if (!$osName) {
            $ua = $this->user_agent ?? '';
    ...
    public function getIsMobileAttribute(): bool
    {
        ...
        // Fallback: check UA for mobile markers
        $ua = $this->user_agent ?? '';
    ...
    public function getIsPhoneAttribute(): bool
    {
        ...
        // Fallback for phones not recognized by DeviceDetector
        $ua = $this->user_agent ?? '';
    ...
    {
        // Only consider it desktop if it fully passes checks
        if ($this->dd()->isDesktop()) {
            // Double check against mobile User-Agent strings
            $ua = $this->user_agent ?? '';
