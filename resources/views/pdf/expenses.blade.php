...
<head>
    <meta charset="UTF-8">
    <title>Reporte de Gastos - Lyrium</title>
    <style>
...
<body>
    <h2>LYRIUM - Reporte de Gastos</h2>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $exp->voucher_type ?? '—' }}</td>
                <td>{{ $exp->receipt_number ?? '—' }}</td>
                <td>{{ $exp->supplier?->name ?? '—' }}</td>
                <td>{{ $exp->concept ?? '—' }}</td>
                <td>{{ optional($exp->issued_at)->format('d/m/Y') }}</td>
                <td class="right">S/ {{ number_format($exp->amount ?? 0, 2) }}</td>
                <td>{{ $exp->status ?? '—' }}</td>
            </tr>
            @endforeach
        </tbody>
    ...
    @if ($expenses->isEmpty())
        <p style="text-align:center;margin-top:20px;color:#999;">No se encontraron gastos.</p>
    @endif
</body>
</html>
